...
class SAINTPluseDataset(Dataset):
...
def get_dataloaders():
    ...
    print("loading csv.....")
    train_df = pd.read_csv(config.train_data_dir, usecols=[
    ...
    train_dataset = SAINTPluseDataset(train, max_seq=config.max_seq)
    val_dataset = SAINTPluseDataset(val, max_seq=config.max_seq)
